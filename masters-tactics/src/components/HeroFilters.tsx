...
export default function HeroFilters({ state, onChange, weaponOptions, moveOptions, originOptions }: Props) {
  const wOpts = useMemo(
    () => ["Any", ...weaponOptions],
    [weaponOptions]
  );
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.5fr repeat(5, 1fr) 1fr", gap: 12, padding: "12px 16px", maxWidth: 960, margin: "0 auto" }}>
        ...
        <select value={state.origin} onChange={(e) => onChange({ origin: e.target.value as any })} style={selStyle}>
          {oOpts.map(o => <option key={o} value={o}>{o}</option>)}
        </select>
